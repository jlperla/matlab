% Modified from: http://people.sc.fsu.edu/~jburkardt/m_src/laguerre_rule/laguerre_rule.m
...
%*****************************************************************************80
%
function [nodes weights] = laguerre_quadrature (order, a)

%  Initialize the parameters.
  alpha = 0.0; %This is a more complicated rule for generalized laguerre quadrature.
  beta = 0.0;
  b = 1.0; %the scaling factor. Could be modified later.
  
%  Construct the rule.
  kind = 5;
  [ nodes, weights ] = cgqf ( order, kind, alpha, beta, a, b );
%
%  Write the rule.
%
  r = zeros ( 2, 1 );
...
global t_cache;
global wts_cache; %global variables to store a cache of the quadrature nodes.  Note worries of threadsafety here....

%  Compute the Gauss quadrature formula for default values of A and B.
%  [ t, wts ] = cdgqf ( nt, kind, alpha, beta ); %THIS IS REPLACED WITH THE CACHED VERSION.
% NOTE:  THE FOLLOWING CODE IS CACHING THE t and wts VARIABLES TO MAX THE ROUTINE MORE EFFICIENT.  IT IS CURRENTLY HARDCODED FOR alpha=0, beta=0, kind=5 (i.e. simply laguerre)
% NOTE: THIS MAY HAVE THREADSAFETY ISSUES, AT WHICH POINT IT COULD MAKE MORE SENSE TO REMOVE THIS FUNCTIONALITY.

if(length(t_cache)== 0 || length(wts_cache)==0) %Create them if they haven't been created yet.  Note that 'exist' doesn't seem to work here since they are globals
    t_cache = containers.Map; %This is a matlab hash table.  Creates a default object.
    wts_cache = containers.Map;
end

 %Check if the value has already been cached.  If it is, then set the t, wts to the saved value.
if(isKey(t_cache, num2str(nt))) %Note that keys seem to require strings.  Had trouble using the integer.
    assert(isKey(wts_cache, num2str(nt))); %Check that it is in both.
    t = t_cache(num2str(nt));
    wts = wts_cache(num2str(nt));    
else
    [ t, wts ] = cdgqf ( nt, 5, 0.0, 0.0);    %Calculates and caches.
    t_cache(num2str(nt)) = t; 
    wts_cache(num2str(nt)) = wts;
end
